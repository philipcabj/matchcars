...
exports.weeklyAgencyDigest = void 0;
...
function statRow(label, value, urgent = false) {
    if (value === 0 && !urgent)
        return "";
...
function buildDigestHtml(agencyName, s) {
    const rows = [
...
exports.weeklyAgencyDigest = (0, scheduler_1.onSchedule)({ schedule: "0 8 * * 1", timeZone: "America/Argentina/Buenos_Aires", region: "us-central1" }, async () => {
    ...
    for (const userDoc of usersSnap.docs) {
        ...
        if (user.weeklyDigestEmail === false)
            continue;
        ...
        try {
            ...
            const activeVehicles = vehiclesSnap.docs
                ...
                .filter((v) => !STOCK_EXCLUDED.includes(v.status || "available"));
            const stats = {
                newLeads: leads.filter((l) => millis(l.createdAt) >= weekAgo).length,
                unanswered: leads.filter((l) => l.status === "new").length,
                pendingOffers: leads.filter((l) => { var _a; return ((_a = l.offer) === null || _a === void 0 ? void 0 : _a.status) === "pending"; }).length,
                won: leads.filter((l) => l.status === "won" && millis(l.wonAt) >= weekAgo).length,
                incompleteStock: activeVehicles.filter((v) => {
                    var _a, _b, _c, _d;
                    const cover = (_d = (_c = (_b = (_a = v.images) === null || _a === void 0 ? void 0 : _a.cover) !== null && _b !== void 0 ? _b : v.coverImage) !== null && _c !== void 0 ? _c : v.cover) !== null && _d !== void 0 ? _d : "";
                    return !cover || !(typeof v.price === "number" && v.price > 0);
                }).length,
                staleStock: activeVehicles.filter((v) => {
                    const days = millis(v.createdAt) ? (now - millis(v.createdAt)) / DAY_MS : 0;
                    return days >= 45 && !leadVehicleIds.has(v.id);
                }).length,
            };
            const hasContent = stats.newLeads || stats.unanswered || stats.pendingOffers || stats.won || stats.incompleteStock || stats.staleStock;
            if (!hasContent)
                continue;
